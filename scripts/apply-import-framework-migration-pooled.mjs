...
const migrationName = "20260626173000_generic_import_framework";
...
async function main() {
  try {
    ...
    const existingTables = await getExistingImportTables();
    ...
    if (existingTables.length > 0 && existingTables.length < 3) {
      throw new Error(
        `Found a partial import-framework schema (${existingTables.join(", ")}). Stop here and inspect Supabase before continuing.`
      );
    }
    ...
    if (existingTables.length === 3) {
      ...
      console.log(`Import-framework tables already exist. Recorded ${migrationName}.`);
      return;
    }
...
async function getExistingImportTables() {
  const rows = await prisma.$queryRawUnsafe(`
    SELECT table_name
    FROM information_schema.tables
    WHERE table_schema = 'public'
      AND table_name IN ('ImportRun', 'ImportRunIssue', 'ImportRunPreviewRow')
    ORDER BY table_name
  `);
